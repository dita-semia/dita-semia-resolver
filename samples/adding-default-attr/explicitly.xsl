...
<xsl:stylesheet version="2.0"
	...
	<xsl:variable name="ATTR_P" as="attribute()">
		<xsl:attribute name="class" select="$CLASS_P"/>
	</xsl:variable>
	
	<xsl:variable name="ATTR_HTML_XREF" as="attribute()+">
		<xsl:attribute name="class" 	select="$CLASS_XREF"/>
    ...
    <xsl:template match="/">
    	...
    	<section class="- topic/section ">
    		...
    		<p class="{$CLASS_P}">You can use constants within you attributes for this as well to avoid the risk of typos.</p>
    		<p class="{$CLASS_P}">There is a file class.xsl that defines constants for (probably) the most common classes.</p>
    		<p>
    		...
    		<ThisIsACustomPElement class="{$CLASS_P}">Note that with this approach the actual name of your elements usually doesn't matter...</ThisIsACustomPElement>
        </section>
