...
<xsl:stylesheet version="2.0"
	...
	xmlns:bc	= "http://www.dita-semia.org/book-cache"
	...
	<xsl:template name="CreateKeySpecContent">
		...
		<xsl:choose>
			<xsl:when test="$key/@akr:ref">
				<xsl:variable name="outputclass"	as="xs:string?"										select="replace($key/@outputclass, '!$', '')"/>
				<xsl:variable name="jKeyDef"		as="jt:org.DitaSemia.Base.AdvancedKeyref.KeyDef?" 	select="akr:getReferencedKeyDef($key)"/>
				<xsl:variable name="href"			as="xs:string?" 									select="if (exists($jKeyDef)) then akr:getKeyDefLocation($jKeyDef, $baseUri) else ()"/>
				<!--<xsl:message>href: <xsl:value-of select="$href"/></xsl:message>-->
				
				<xsl:variable name="displaySuffix" 	as="xs:string*" select="akr:getKeyRefDisplaySuffix($key, $jKeyDef)"/>
						...
						<xsl:otherwise>
							<xsl:variable name="keyContent" as="node()*">
								<xsl:call-template name="KeyFormatting">
									<xsl:with-param name="keyNode" select="$key"/>
				...
				<xsl:choose>
					...
					<xsl:otherwise>
						<xsl:sequence select="$refContent"/>
						<xsl:if test="empty($jKeyDef) and not(ancestor-or-self::*[ds:getCbaFlags(.) = $CBA_FLAG_HIDE])(: and (bc:isUrlInBookCache(base-uri(.)):)">
							<xsl:call-template name="DotXMessage">
								<xsl:with-param name="type"	select="'WARN'"/>
								<xsl:with-param name="message">Failed to resolve advanced-keyref to xref (<xsl:value-of select="$key/@akr:ref"/>).</xsl:with-param>
							</xsl:call-template>
						</xsl:if>
					</xsl:otherwise>
				</xsl:choose>
			</xsl:when>
			<xsl:when test="$key/@ikd:key-type">
				<xsl:call-template name="KeyFormatting">
					<xsl:with-param name="keyNode" select="$key"/>
					<xsl:with-param name="content" select="$content"/>
				</xsl:call-template>
